...
rezervace_blp = Blueprint("rezervace", __name__,
                          url_prefix="/api/v1/rezervace")


class RezervaceFilterSchema(Schema):
...
class RezervaceResource(MethodView):
    ...
    def get(self, query_args):
        ...
        return dotaz.order_by(Rezervace.rezervovane_datum).all()
...
@rezervace_blp.route("/vyber/")
class RezervaceVyberResource(MethodView):
    @rezervace_blp.response(200, RezervaceSchema(many=True))
    def get(self):
        ...
        if od:
            try:
                od_datum = datetime.strptime(od, "%Y-%m-%d").date()
                dotaz = dotaz.filter(Rezervace.rezervovane_datum >= od_datum)
            except ValueError:
                abort(400, message="Neplatný formát parametru 'od' (YYYY-MM-DD)")

        if do:
            try:
                do_datum = datetime.strptime(do, "%Y-%m-%d").date()
                dotaz = dotaz.filter(Rezervace.rezervovane_datum <= do_datum)
            except ValueError:
                abort(400, message="Neplatný formát parametru 'do' (YYYY-MM-DD)")
